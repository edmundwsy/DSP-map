...
void                                cloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud) {
  ...
  occ_grid_msg.info.height     = MAP_WIDTH_VOXEL_NUM;
  occ_grid_msg.info.width      = MAP_LENGTH_VOXEL_NUM;
  occ_grid_msg.info.origin.position.x = uav_position.x() - (MAP_LENGTH_VOXEL_NUM - 1) / 2 * res ;
  occ_grid_msg.info.origin.position.y = uav_position.y() - (MAP_WIDTH_VOXEL_NUM - 1) / 2 * res;
